...
class StorageHandler:
    ...
    def get_config_value(self, key: str, default=None):
        """
        Lê uma configuração do namespace oficial e faz fallback transparente
        para a chave legada sem namespace. Quando encontra valor legado,
        migra para o namespace oficial.
        """
        namespaced_key = self._get_redis_key(key)
        value = self.redis.get(namespaced_key)
        if value is not None:
            return value

        legacy_key = self._get_legacy_key(key)
        legacy_value = self.redis.get(legacy_key)
        if legacy_value is not None:
            self.redis.set(namespaced_key, legacy_value)
            return legacy_value

        return default

    def set_config_value(self, key: str, value):
        """Salva uma configuração no namespace oficial."""
        self.redis.set(self._get_redis_key(key), value)

    def initialize_default_config(self, api_domain: str = None):
        """Garante que as configurações mínimas existam no namespace oficial."""
        defaults = {
            ...
            "summary_header": "🤖 *Resumo do áudio:*",
            "transcription_header": "🔊 *Transcrição do áudio:*",
            "output_mode": "both",
            "character_limit": "500",
        ...
        if api_domain:
            defaults["api_domain"] = api_domain

        for key, value in defaults.items():
            self.redis.set(self._get_redis_key(key), self.get_config_value(key, value))
    ...
    def get_message_settings(self):
        ...
        return {
            "summary_header": self.get_config_value("summary_header", "🤖 *Resumo do áudio:*"),
            "transcription_header": self.get_config_value("transcription_header", "🔊 *Transcrição do áudio:*"),
            "output_mode": self.get_config_value("output_mode", "both"),
            "character_limit": int(self.get_config_value("character_limit", "500")),
        }
    ...
    def get_process_mode(self):
        ...
        mode = self.get_config_value("process_mode", "all")
    ...
    def get_auto_language_detection(self) -> bool:
        ...
        """
        return self.get_config_value("auto_language_detection", "false") == "true"

    def set_auto_language_detection(self, enabled: bool):
        """
        Ativa ou desativa a detecção automática de idioma
        """
        self.set_config_value("auto_language_detection", str(enabled).lower())
    ...
    def get_auto_translation(self) -> bool:
        ...
        """
        return self.get_config_value("auto_translation", "false") == "true"

    def set_auto_translation(self, enabled: bool):
        """
        Ativa ou desativa a tradução automática
        """
        self.set_config_value("auto_translation", str(enabled).lower())
    ...
    def get_llm_provider(self) -> str:
        ...
        return self.get_config_value("active_llm_provider", "groq")
    
    def set_llm_provider(self, provider: str):
        """Sets active LLM provider"""
        if provider not in ["groq", "openai"]:
            raise ValueError("Provider must be 'groq' or 'openai'")
        self.set_config_value("active_llm_provider", provider)
    ...
    def get_business_message(self) -> str:
        return self.get_config_value("business_message", "*Impacte AI* Premium Services")

    def get_transcription_language(self) -> str:
        return self.get_config_value("transcription_language", "pt")

    def get_output_settings(self) -> Dict:
        settings = self.get_message_settings()
        settings["use_timestamps"] = self.get_config_value("use_timestamps", "false") == "true"
        return settings

    def get_runtime_flags(self) -> Dict:
        return {
            "process_group_messages": self.get_config_value("process_group_messages", "false") == "true",
            "process_self_messages": self.get_config_value("process_self_messages", "true") == "true",
            "debug_mode": self.get_config_value("debug_mode", "false") == "true",
        }

    def set_api_domain(self, api_domain: str):
        self.set_config_value("api_domain", api_domain)

    def get_api_domain(self) -> str:
        return self.get_config_value("api_domain", "seu.dominio.com")
