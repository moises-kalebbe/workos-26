...
export default function VaultPage() {
  ...
  if (!user) return <LoadingState message="Autenticacao necessaria." />;
        ...
        <div className="space-y-6">
          ...
          <Tabs value={activeTab} onValueChange={(value) => setActiveTab(value as VaultHubTab)} className="space-y-6">
            <div className="flex flex-col gap-3 lg:flex-row lg:items-center lg:justify-between">
              <TabsList className="h-auto flex-wrap justify-start gap-2 bg-transparent p-0"><TabsTrigger value="overview">Visao geral</TabsTrigger><TabsTrigger value="credentials">Credenciais</TabsTrigger><TabsTrigger value="repositories">Repositorios</TabsTrigger><TabsTrigger value="environments">Envs</TabsTrigger><TabsTrigger value="imports">Importacoes</TabsTrigger></TabsList>
            ...
            <TabsContent value="credentials" className="space-y-4">
              ...
              <div className="grid gap-4 xl:grid-cols-2">
                {filteredCredentials.map((entry) => {
                  ...
                  return <Card key={entry.id} className="border-border/70 bg-card/70"><CardHeader className="flex flex-row items-start justify-between gap-3 space-y-0"><div><CardTitle className="text-lg">{entry.service || "Servico sem nome"}</CardTitle><CardDescription>{entry.client || GENERAL_PROJECT_LABEL}</CardDescription></div><div className="flex items-center gap-2"><Button variant="outline" size="icon" onClick={() => openCredentialDialog(entry)}><Pencil className="h-4 w-4" /></Button><Button variant="outline" size="icon" onClick={() => setDeleteCredential(entry)}><Trash2 className="h-4 w-4" /></Button></div></CardHeader><CardContent className="space-y-4"><div className="grid gap-3 sm:grid-cols-2"><div><p className="text-xs text-muted-foreground">Usuario</p><p className="text-sm">{entry.username || "-"}</p></div><div><p className="text-xs text-muted-foreground">URL</p><p className="truncate text-sm">{entry.url || "-"}</p></div></div><div className="rounded-xl border border-border/60 bg-background/40 p-3"><div className="flex items-center justify-between gap-3"><div><p className="text-xs text-muted-foreground">Senha</p><p className="font-mono text-sm">{secretVisible ? password || "-" : password ? "****************" : "-"}</p></div><div className="flex items-center gap-2"><Button variant="outline" size="icon" onClick={() => toggleSecret(secretKey)}>{secretVisible ? <EyeOff className="h-4 w-4" /> : <Eye className="h-4 w-4" />}</Button><Button variant="outline" size="icon" onClick={() => void copyToClipboard(password, "Senha")}><Copy className="h-4 w-4" /></Button></div></div></div><p className="text-sm text-muted-foreground">{entry.notes || "Sem observacoes."}</p></CardContent></Card>;
                })}
              </div>
            </TabsContent>

            <TabsContent value="repositories" className="space-y-4">
              <Card className="border-border/70 bg-card/70"><CardHeader className="flex flex-col gap-4 lg:flex-row lg:items-center lg:justify-between"><div><CardTitle>Integracao GitHub</CardTitle><CardDescription>Conecte um token do GitHub para importar repositorios remotos.</CardDescription></div><div className="flex flex-wrap gap-2"><Button variant="outline" onClick={() => setGithubDialogOpen(true)}><Plus className="mr-2 h-4 w-4" />{activeGithubConnection ? "Trocar token GitHub" : "Conectar GitHub"}</Button><Button onClick={() => void syncGithub()} disabled={!activeGithubConnection || mutating}><RefreshCcw className="mr-2 h-4 w-4" />Sincronizar GitHub</Button></div></CardHeader><CardContent>{activeGithubConnection ? <div className="grid gap-3 sm:grid-cols-4"><div className="rounded-xl border border-border/60 bg-background/40 p-4"><p className="text-xs text-muted-foreground">Conta</p><p className="mt-1 text-sm font-medium">{activeGithubConnection.github_login}</p></div><div className="rounded-xl border border-border/60 bg-background/40 p-4"><p className="text-xs text-muted-foreground">Conexao</p><p className="mt-1 text-sm font-medium">{activeGithubConnection.display_name}</p></div><div className="rounded-xl border border-border/60 bg-background/40 p-4"><p className="text-xs text-muted-foreground">Ultimo sync</p><p className="mt-1 text-sm font-medium">{formatDateTime(activeGithubConnection.last_synced_at)}</p></div><div className="rounded-xl border border-border/60 bg-background/40 p-4"><p className="text-xs text-muted-foreground">Scopes</p><p className="mt-1 text-sm font-medium">{activeGithubConnection.scopes?.join(", ") || "-"}</p></div></div> : <p className="text-sm text-muted-foreground">Nenhuma conexao GitHub configurada ainda.</p>}</CardContent></Card>
